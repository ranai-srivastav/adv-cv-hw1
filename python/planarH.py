...
def computeH_norm(x1, x2) -> np.ndarray:
    ...
    T1 = np.array([
                    [                         np.sqrt(2) / x1_max[0],                                          0,  0],
                    [                                              0,                     np.sqrt(2) / x1_max[1],  0],
                    [     -1 * np.sqrt(2) * x1_centroid[0]/x1_max[0], -1 * np.sqrt(2) * x1_centroid[1]/x1_max[1],  1]])
    x1_norm = T1 @ x1.T

    # Similarity transform 2
    T2 = np.array([[np.sqrt(2) / x2_max[0],                      0,               0,
                   [                     0, np.sqrt(2) / x2_max[1], 0],
                   [-1 * np.sqrt(2) * x2_centroid[0]/x2_max[0]], -1 * np.sqrt(2) * x2_centroid[1]/x2_max[1], 1]])
    x2_norm = T2 @ x2.T
